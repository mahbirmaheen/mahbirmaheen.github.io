...
function parseAndLoadData() {
    ...
    } else { alert("Please select a file or paste data."); }
}
...
function processRawData(data) {
    ...
    for (let i = 0; i < firstRow.length; i++) {
        if (hasHeaders && firstRow[i] !== null) { dataHeaders.push(firstRow[i].toString().trim()); } 
        else { dataHeaders.push(`Column ${i + 1}`); }
    }
    let xHtml = ''; let yHtml = '';
...
function generatePlot() {
    ...
    rawYDatasets.forEach((yData, index) => {
        ...
        traces.push({
            x: visualX, y: visualY, mode: 'markers', type: 'scatter', name: yData.name + ' (Data)',
        ...
        if (fitType === 'spline') {
            traces.push({
                x: validPairs.map(p => p[0]), y: validPairs.map(p => p[1]), mode: 'lines', type: 'scatter', name: yData.name + ' (Strict Spline)',
                line: { color: color, width: 2.5, shape: 'spline', smoothing: 1.3 }, hoverinfo: 'skip'
            });
            ...
            let curveX = [], curveY = [];
            let xMin = Math.min(...validPairs.map(p=>p[0])); let xMax = Math.max(...validPairs.map(p=>p[0]));
            let curveStep = (xMax - xMin) / 200; 
            for(let x = xMin; x <= xMax; x += curveStep) {
                curveX.push(x); curveY.push(result.predict(x)[1]);
            }
            traces.push({
                x: curveX, y: curveY, mode: 'lines', type: 'scatter', name: yData.name + ' (Fit)',
                line: { color: color, width: 2.5, dash: 'solid' }, hoverinfo: 'skip'
            });
            rSquaredText += `&bull; <b style="color:${color}">${yData.name}:</b> R² = ${result.r2} <br><span style="font-family:monospace; font-size:13px; color:#4a5568; margin-left:15px; display:block; margin-bottom: 8px;">Eq: ${result.string}</span>`;
        }
    });

    const layout = {
        title: { text: document.getElementById('graphTitle').value, font: { size: scale.titleSize, family: 'Arial', color: '#1a202c', weight: 'bold' } },
        width: exportWidth, height: exportHeight, plot_bgcolor: "#fafafa", paper_bgcolor: "white", margin: { l: 80, r: scale.rightMargin, t: 80, b: 80 }, 
        xaxis: { title: { text: xAxisTitle, font: { size: scale.axisSize, weight: 'bold' } }, showgrid: true, gridcolor: '#edf2f7', zeroline: false, showline: true, linewidth: 2, linecolor: '#2d3748', mirror: 'ticks', ticks: 'inside' },
        yaxis: { title: { text: yAxisTitle, font: { size: scale.axisSize, weight: 'bold' } }, showgrid: true, gridcolor: '#edf2f7', zeroline: false, showline: true, linewidth: 2, linecolor: '#2d3748', mirror: 'ticks', ticks: 'inside' },
        legend: { orientation: 'v', x: 1.02, y: 1, xanchor: 'left', yanchor: 'top', bordercolor: '#e2e8f0', borderwidth: 1, bgcolor: '#ffffff', font: { size: scale.legendSize } }, hovermode: 'closest'
    };
    ...
    Plotly.newPlot('plotContainer', traces, layout, config).then(() => {
        document.getElementById('r-squared-container').innerHTML = rSquaredText;
    });
}